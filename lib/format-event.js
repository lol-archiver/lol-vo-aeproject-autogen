const formatCondition = (params) => {
	...
	// 行为Only
		...
		'选用', '禁用',
		'初遇', '再遇', '遇见', '攻击', '击杀', '抢先击杀', '摧毁', '治疗', '阵亡', '重生', '出生', '助攻', '占据',
		'普攻', '暴击', '使用', '进化', '学习', '升级', '初次升级', '触发', '冷却', '等级提升', '技能',
		'打开', '关闭', '浏览',
		'接近', '抵达', '购买', '附近', '远处', '变身',
		'玩笑', '嘲讽', '跳舞', '大笑', '静置', '回城',
		'自己', '友方', '敌方', '玩家',
	...
	// 内容可选+大括号
		...
		'武器', '道具', '强化', '女性', '男性', '首杀', '多杀', '连杀', '累杀', '团灭', '控制', '商店',
	].includes(type)) {
		if(['连杀'].includes(type)) {
			result += `${type}【${main}】`;
		}
	...
	// 内容必要
	else if(['系列', '宇宙', '地区', '种族', '特征', '造型', '位置', '定位', '动作', '被', '血量', '备注', '注释', '标题', '对局', '信号', '获得', '数量', '野怪营地'].includes(type)) {
		...
		// 地区/种族/位置/定位
		else if(['地区', '种族', '位置', '定位'].includes(type)) {
			result += `【${main}】英雄`;
		}
		// 信号
		...
		// 时3
			...
			if(params[1]) {
				result += `${type}${formatCondition(params.slice(1))}${main}`;
