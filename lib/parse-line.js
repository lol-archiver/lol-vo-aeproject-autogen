...
const parseLine = async (lineRaw, event = {}, namesAudio = [], extrasLine = {}, linesRaw = []) => {
	...
	let file = lineRaw.file;
	if(!file && lineRaw.folder) {
		if(lineRaw.file) {
			file = resolve(C.dirExtract, '@final', lineRaw.folder, lineRaw.file);
		}
		else {
			const dirAudio = resolve(C.dirExtract, '@final', parseConfig(lineRaw.folder));
