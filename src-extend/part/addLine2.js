P.addLine = function addLine(line, lid, dirLine, duration) {
	...
	var layerPictureMain = compLine.layers.add(F(T.parseConfig(line.head), T.dirFootage));

	var layerCircleTarget = hasTarget ? compLine.layers.addShape() : null;
	var layerPictureTarget = hasTarget ? compLine.layers.add(F(T.parseConfig(hasTarget), T.dirFootage)) : null;

	var layerPictureSkill = hasSkill ? compLine.layers.add(F(T.parseConfig(hasSkill), T.dirFootage)) : null;
