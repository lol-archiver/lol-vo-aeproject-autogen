export default {
	emote: '…(⊙＿⊙；)…',
	event: {
		'[初遇:英雄:阿木木]':
			{ target: '${C.path.project.autogen}reso/icon/Amumu/0.png' },
		'[击杀:英雄:阿木木]':
			{ target: '${C.path.project.autogen}reso/icon/Amumu/0.png' },
		'[初遇:英雄:阿兹尔]':
			{ target: '${C.path.project.autogen}reso/icon/Azir/0.png' },
		'[击杀:英雄:阿兹尔]':
			{ target: '${C.path.project.autogen}reso/icon/Azir/0.png' },
		'[初遇:英雄:德莱文]':
			{ target: '${C.path.project.autogen}reso/icon/Draven/0.png' },
		'[击杀:英雄:德莱文]':
			{ target: '${C.path.project.autogen}reso/icon/Draven/0.png' },
		'[初遇:英雄:伊泽瑞尔]':
			{ target: '${C.path.project.autogen}reso/icon/Ezreal/0.png' },
		'[击杀:英雄:伊泽瑞尔]':
			{ target: '${C.path.project.autogen}reso/icon/Ezreal/0.png' },
		'[初遇:英雄:乐芙兰]':
			{ target: '${C.path.project.autogen}reso/icon/LeBlanc/0.png' },
		'[击杀:英雄:乐芙兰]':
			{ target: '${C.path.project.autogen}reso/icon/LeBlanc/0.png' },
		'[初遇:英雄:厄运小姐]':
			{ target: '${C.path.project.autogen}reso/icon/MissFortune/0.png' },
		'[击杀:英雄:厄运小姐]':
			{ target: '${C.path.project.autogen}reso/icon/MissFortune/0.png' },
		'[初遇:英雄:拉莫斯]':
			{ target: '${C.path.project.autogen}reso/icon/Rammus/0.png' },
		'[击杀:英雄:拉莫斯]':
			{ target: '${C.path.project.autogen}reso/icon/Rammus/0.png' },
		'[初遇:英雄:希维尔]':
			{ target: '${C.path.project.autogen}reso/icon/Sivir/0.png' },
		'[击杀:英雄:希维尔]':
			{ target: '${C.path.project.autogen}reso/icon/Sivir/0.png' },
		'[初遇:英雄:斯维因]':
			{ target: '${C.path.project.autogen}reso/icon/Swain/0.png' },
		'[击杀:英雄:斯维因]':
			{ target: '${C.path.project.autogen}reso/icon/Swain/0.png' },
		'[初遇:英雄:锤石]':
			{ target: '${C.path.project.autogen}reso/icon/Thresh/0.png' },
		'[击杀:英雄:锤石]':
			{ target: '${C.path.project.autogen}reso/icon/Thresh/0.png' },
		'[初遇:英雄:弗拉基米尔]':
			{ target: '${C.path.project.autogen}reso/icon/Vladimir/0.png' },
		'[击杀:英雄:弗拉基米尔]':
			{ target: '${C.path.project.autogen}reso/icon/Vladimir/0.png' },
		'[初遇:英雄:泽拉斯]':
			{ target: '${C.path.project.autogen}reso/icon/Xerath/0.png' },
		'[击杀:英雄:泽拉斯]':
			{ target: '${C.path.project.autogen}reso/icon/Xerath/0.png' },


		'[使用:Q技能:交火:远程射击]':
			{ skill: '${C.path.project.autogen}reso/icon/Samira/Q.png' },
		'[使用:Q技能:交火:近战斩击]':
			{ skill: '${C.path.project.autogen}reso/icon/Samira/Q.png' },
		'[使用:W技能:锋旋]':
			{ skill: '${C.path.project.autogen}reso/icon/Samira/W.png' },
		'[使用:E技能:狂飙]':
			{ skill: '${C.path.project.autogen}reso/icon/Samira/E.png' },
		'[使用:R技能:炼狱扳机:可以使用]':
			{ skill: '${C.path.project.autogen}reso/icon/Samira/R_8.png' },
		'[使用:R技能:炼狱扳机]':
			{ skill: '${C.path.project.autogen}reso/icon/Samira/R_8.png' },
	},
	lines: {
		'2CA02935': [
			{
				oper: 'before',
				side: 'left',
				duration: 0.4,
				head: '${C.path.project.autogen}reso/icon/Amumu/0.png',
				line: 'ヾ(Ő∀Ő๑)ﾉ',
			},
			{
				oper: 'after',
				side: 'left',
				duration: 0.4,
				head: '${C.path.project.autogen}reso/icon/Amumu/0.png',
				line: '(╥﹏╥)',
			},
		],

		'86EBEB1F': [
			{
				oper: 'extra',
				mark: '引申自国外一则关于性别歧视的舆论：不爱笑的女性容易受到男性的批评\n仅 烬/德莱厄斯/洛/崔斯特/杰斯/古拉加斯/格雷福斯/塞拉斯 可触发'
			}
		],
		'89020769': [
			{
				oper: 'extra',
				mark: '“女王”引申自莎弥拉的开发代号“Queen”'
			}
		],
		'9FEA7D3D': [
			{
				oper: 'extra',
				mark: '原文“I’m the rose that survived the desert.”\n除了引申自莎弥拉的英雄称号“沙漠玫瑰”外\n还致敬了已故歌手2PAC的诗词\n《盛开在钢筋水泥中的玫瑰（The Rose That Grew from Concrete）》'
			}
		],
		'7D8B2520': [
			{
				oper: 'extra',
				mark: '原文意思是“排名根据心烦程度分先后”，估计是太顺口念错了'
			}
		],
		'C7D2A920': [
			{
				oper: 'extra',
				mark: '原文“Like Tereshni all over again”\n致敬了漫威电影宇宙中黑寡妇对鹰眼的台词\n“This is just like Budapest all over again”'
			}
		],
		'0C658311': [
			{
				oper: 'extra',
				mark: '“中了！”原文“Jackpot!”，致敬了游戏《鬼泣1》中但丁击杀魔帝时的台词'
			}
		],
	}
};