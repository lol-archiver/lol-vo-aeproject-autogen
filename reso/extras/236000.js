module.exports = {
	emote: '{{{(>_<)}}}',
	event: {
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话1-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话1-3]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话2-2]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话3-2]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话4-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话4-3]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话5-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话5-3]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话6-2]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话7-2]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话7-4]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[移动:初次][附近:友方:使用:英雄:赛娜:注释:对话8-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[被:治疗:英雄:赛娜:注释:对话9-2]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png',
		},
		'[被:治疗:英雄:赛娜:注释:对话10-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png',
		},
		'[被:治疗:英雄:赛娜:注释:对话11-2]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png',
		},
		'[击杀:英雄][附近:友方:使用:英雄:赛娜:注释:对话12-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png',
		},
		'[击杀:英雄][附近:友方:使用:英雄:赛娜:注释:对话13-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[击杀:英雄][附近:友方:使用:英雄:赛娜:注释:对话14-1]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[初遇:英雄:锤石][附近:友方:使用:英雄:赛娜:注释:对话15-2]': {
			target: '${C.path.project.autogen}reso/icons/Thresh/0.png'
		},

		'[初遇:英雄:赛娜]': {
			target: '${C.path.project.autogen}reso/icons/Senna/0.png'
		},
		'[初遇:英雄:锤石]': {
			target: '${C.path.project.autogen}reso/icons/Thresh/0.png'
		},
		'[初遇:英雄:约里克]': {
			target: '${C.path.project.autogen}reso/icons/Yorick/0.png'
		},
		'[初遇:英雄:赫卡里姆]': {
			target: '${C.path.project.autogen}reso/icons/Hecarim/0.png'
		},
		'[初遇:英雄:薇恩]': {
			target: '${C.path.project.autogen}reso/icons/Vayne/0.png'
		},
		'[初遇:英雄:古拉加斯]': {
			target: '${C.path.project.autogen}reso/icons/Gragas/0.png'
		},
		'[初遇:英雄:厄运小姐]': {
			target: '${C.path.project.autogen}reso/icons/MissFortune/0.png'
		},
		'[初遇:英雄:莫德凯撒]': {
			target: '${C.path.project.autogen}reso/icons/Mordekaiser/0.png'
		},


		'[初遇:地区:德玛西亚]': {
			target: '${C.path.project.autogen}reso/regions/demacia.png'
		},
		'[初遇:地区:暗影岛]': {
			target: '${C.path.project.autogen}reso/regions/shadow_isles.png'
		},

		'[使用:W技能:热诚烈弹]': {
			skill: '${C.path.project.autogen}reso/icons/Lucian/W.png'
		},
		'[使用:E技能:冷酷追击]': {
			skill: '${C.path.project.autogen}reso/icons/Lucian/E.png'
		},
		'[使用:R技能:圣枪洗礼]': {
			skill: '${C.path.project.autogen}reso/icons/Lucian/R.png'
		},
	},
	lines: {
		'EDB35506': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			mark: '对话1',
		},
		'80BD0409': {
			hideEvent: true,
			befores: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '噩梦一场，暗无天日，饭吃得还行',
					mark: '原文“Food was great through”。是“饭”，不是“但”',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'F1CEFE45',
					hideEvent: true
				},
			],
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '假的',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '71150077',
					hideEvent: true
				},
			],
		},

		'472BBE76': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			befores: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '你怎么不说话？',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'E0670144',
					mark: '对话2',
					hideEvent: true,
				},
			],
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '再说这种话我就咒死你',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '1E93BA33',
					hideEvent: true
				},
			],
		},

		'F69D3355': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			befores: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '要怎么做？',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'EE7E102C',
					hideEvent: true,
					mark: '对话3',
				},
			],
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '目前看来很管用',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '21316636',
					hideEvent: true,
				},
			],
		},

		'F6BE0B05': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			mark: '对话4',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '啊，是啊，我在那边遇到了别人\n他是个尸鬼，我们相爱，还生下了尸鬼娃娃',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'AF52701E',
					hideEvent: true
				},
			]
		},
		'CADFB73F': {
			hideEvent: true,
		},

		'847B7B6F': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			mark: '对话5',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '会用，就不觉得沉',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'FEC86A61',
					hideEvent: true
				},
			]
		},
		'359BA905': {
			mark: '原文“Cause I tried to... Ohhh”。是“试”，不是“是”',
			hideEvent: true,
		},

		'DEAB5312': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			befores: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '准备好了吗？',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'E001386F',
					hideEvent: true,
					mark: '对话6',
				},
			],
		},

		'B6E41D6F': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			befores: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '别拖后腿',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '1435996C',
					hideEvent: true,
					mark: '对话7',
				},
			],
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '你想听我尖叫吗？',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '1C204C24',
					hideEvent: true,
				},
			],
		},
		'71F2E946': {
			hideEvent: true,
		},

		'3674E411': {
			event: '[移动:初次][附近:友方:使用:英雄:赛娜]',
			mark: '对话8',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '一起清算',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '193E6E6A',
					hideEvent: true
				},
			]
		},

		'45E97412': {
			event: '[英雄:赛娜:治疗:自己]',
			befores: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					mark: '对话9',
					line: '生命来自死亡',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'D2A3566B',
					hideEvent: true
				},
			]
		},

		'61DE1357': {
			event: '[英雄:赛娜:治疗:自己]',
			mark: '对话10',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '我是在瞄准他们',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'AF59D07C',
					hideEvent: true
				},
			]
		},

		'4F076C03': {
			event: '[英雄:赛娜:治疗:自己]',
			mark: '对话11',
			befores: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '你给我活着，才能吻我',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '67037B68',
					hideEvent: true
				},
			]
		},

		'5AA51077': {
			event: '[击杀:英雄][附近:友方:使用:英雄:赛娜]',
			mark: '对话12',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '就像那些旧的回忆一样',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'AEE95762',
					hideEvent: true
				},
			]
		},

		'DC062B59': {
			event: '[击杀:英雄][附近:友方:使用:英雄:赛娜]',
			mark: '对话13',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '以及所有迷失在黑暗中的人',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'DC958402',
					hideEvent: true
				},
			]
		},

		'8E7E8478': {
			event: '[击杀:英雄][附近:友方:使用:英雄:赛娜]',
			mark: '对话14',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '你真贴心',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: 'CF258357',
					hideEvent: true
				},
			]
		},

		'73F1DF2F': {
			event: '[初遇:英雄:锤石][附近:友方:使用:英雄:赛娜]',
			mark: '对话15',
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '不，我们一起来',
					folder: '[235000]涤魂圣枪 赛娜[Senna@PBE1@zh_cn]',
					hash: '038B6E3B',
					hideEvent: true
				},
			]
		},

		'4684582B': {
			afters: [
				{
					side: 'left',
					head: '${C.path.project.autogen}reso/icons/Senna/0.png',
					line: '(σ｀д′)σ',
					hideEvent: true
				},
			]
		},

		'341B9A3E': {
			mark: '原文“Two in the head.”。是“头”，不是“偷”'
		},


		'C0FD2C10': {
			mark: '仅 凯特琳/格雷福斯/烬/金克斯/卡莎/卢锡安/厄运小姐/崔丝塔娜/厄加特 可触发'
		},
		'F975D93C': {
			mark: '仅 凯特琳/格雷福斯/烬/金克斯/卡莎/卢锡安/厄运小姐/崔丝塔娜/厄加特 可触发'
		},
		'FACF412A': {
			mark: '仅 凯特琳/格雷福斯/烬/金克斯/卡莎/卢锡安/厄运小姐/崔丝塔娜/厄加特 可触发'
		},
	}
};